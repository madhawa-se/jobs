                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                        ...
                                        if (isset($employer_post) && $employer_post != '') {
                                            ...
                                            // getting job posted date  New edit
                                             $cs_job_posted_date = get_post_meta($job_post->ID, 'cs_job_posted', true);
											
											   $cs_job_expired = get_post_meta($job_post->ID, 'cs_job_expired', true);
                                            ...
                                            if ($cs_job_posted_date != '') {
											 ...
											 if ($user_role == 'cs_employer') {
											   ...
											   if ($cs_job_expired != '') {
                                                $jobs_info_output .= '<li>';
                                                $jobs_info_output .= '<i class="icon-calendar5"></i>';
                                                $jobs_info_output .= __('Ad Expire Date:', 'jobhunt') . '  <span>' . date_i18n(get_option('date_format'), $cs_job_expired) . '</span>';
                                                $jobs_info_output .= '</li>';
                                            }
											}
                                            ...
                                            ?>
                                            <form action="<?php echo site_url(); ?>/pdf" method="post" id="pdf_download_form">
                                                <input type="hidden" name="com_img" value="<?php echo esc_url($cs_employee_employer_img); ?>" />
                                                <input type="hidden" name="nm_pdf_views" value="<?php echo esc_attr($cs_count_views); ?>" />
                                                <input type="hidden" name="nm_pdf_app" value="<?php echo esc_html($applications); ?>" />
                                                <input type="hidden" name="nm_pdf_add_exp" value="<?php echo date_i18n(get_option('date_format'), $cs_job_expired); ?>" />
                                                <input type="hidden" name="nm_pdf_post_date" value="<?php echo date_i18n(get_option('date_format'), $cs_job_posted_date); ?>" />
                                                <?php 
                                                $cs_application_closing_date = get_post_meta($job_post->ID, 'cs_application_closing_date', true)
                                                ?>
                                                <input type="hidden" name="nm_pdf_apply_befor" value="<?php echo date_i18n(get_option('date_format'), $cs_application_closing_date); ?>" />
                                                ...
                                                <input type="hidden" name="nm_pdf_content" value="<?php echo strip_tags($content); ?>" />
                                                <input type="hidden" name="nm_pdf_com_name" value="<?php echo esc_attr($username); ?>" />
                                                <input type="hidden" name="nm_pdf_job_ref" value="<?php echo $job_post->ID; ?>" />
                                            </form>
                                        <?php } ?>
                                    </div>
                                </div>
                    ...
                    <div class="<?php if (isset($cs_plugin_options['cs_plugin_single_container']) && $cs_plugin_options['cs_plugin_single_container'] == 'on') echo 'container' ?>">
                        <div class="row">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
                                        ...
                                        <div class="apply-buttons">
                                            ...
                                            <?php } else {
                                                ...
                                                <a class="cs-add-wishlist btn small" href="javascript:void(0)"  id="<?php echo 'addjobs_to_wishlist' . intval($job_post->ID); ?>" 
                                                   onclick="show_alert_msg('<?php echo __("Become a candidate then try again.", "jobhunt"); ?>')" >
                                                    ...
                                                    <a class="cs-add-wishlist btn small" href="#" onclick="document.getElementById('pdf_download_form').submit();">
                                                            <i class=""></i><?php _e('Download pdf', 'jobhunt'); ?>
                                                        </a>
                                                <?php
                                            }
                                            $class_apply = '';
                                            ...
                                            if (is_user_logged_in()) {
                                                $user = cs_get_user_id();
                                               echo  $user_role = cs_get_loginuser_role();
                            ...
                            <!--                                                            <a class="active btn large like applied_icon" onclick="cs_removejobs_left_to_applied('<?php echo esc_url(admin_url('admin-ajax.php')); ?>', '<?php echo intval($job_post->ID); ?>', this)" >
                                                            ...
                                                            ?>
                                                            <a class="active btn large like applied_icon <?php echo $class_apply; ?>" onclick="cs_addjobs_left_to_applied('<?php echo esc_url(admin_url('admin-ajax.php')); ?>', '<?php echo intval($job_post->ID); ?>', this)" >
